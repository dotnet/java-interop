...
namespace Java.Interop.Tools.Common.Enumification
{
	...
	{
		public static ConstantEntryComparer Comparer { get; } = new ConstantEntryComparer ();
		...
		public class ConstantEntryComparer : IComparer<ConstantEntry>
		{
			public int Compare (ConstantEntry x, ConstantEntry y)
			{
				// If they have matching EnumFullType, sort by value
				if (x.EnumFullType == y.EnumFullType || x.JavaSignature == y.JavaSignature)
					return string.CompareOrdinal (x.Value, y.Value);

				// Sort by JavaSignature if possible
				if (x.JavaSignature.HasValue () && y.JavaSignature.HasValue ())
					return string.CompareOrdinal (x.JavaSignature, y.JavaSignature);

				// Else there isn't really much to compare them on :shrug:
				return 0;
			}
		}
	}
