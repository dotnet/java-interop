...
namespace MonoDroid.Generation
{
	...
	public class GeneratedEnumSymbol : ISymbol
	{
		...
		{
			if (IsArray)
				return String.Format ("({0}[]{3}) JNIEnv.GetArray<{0}> ({1}, {2})", opt.GetOutputName (enum_type), var_name, owned ? "JniHandleOwnership.TransferLocalRef" : "JniHandleOwnership.DoNotTransfer", opt.NullableOperator);
			else
